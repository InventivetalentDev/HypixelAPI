...
import { Callback, WebhookResult } from "./util";
...
function doPost(context: string, data: any, url: string, format: string, extraOptions_: any, connection: Connection, targetId: number) {
    ...
    post({
        url: url,
        json: postData
    }, function (err: Error, response: Response, body) {
        if (err) {
            console.warn(err);
            return;
        }
        ...
        if (response.statusCode < 200 || response.statusCode > 230) {// error
            connection.query("UPDATE skyblock_webhooks SET errorCounter = errorCounter+1 WHERE id=?", [targetId], function (_, __) {
            })
        } else {// success
            connection.query("UPDATE skyblock_webhooks SET successCounter = successCounter+1 WHERE id=?", [targetId], function (_, __) {
            })
        }
    });
}

export default function(pool: Pool): WebhookResult {
    let stack: any[] = [];

    let queryWebhooks = function (context: string, cb: Callback) {
    ...
    return {
        ...
        queryWebhooksAndRun: function (context: string, data_: any) {
            if (data_) {
                data.push(data_);
            }
