...
import { simplifyNbtMap } from "../../nbtHelper";
import { verifyMinecraftUsername } from "../../util";
...
export default function(_: Vars, pool: Pool) {
    return function (req: Request, res: Response) {
        ...
        pool.getConnection(function (err, connection) {
            ...
            verifyMinecraftUsername(username, function (_, nameRes) {
                if (nameRes) {
                    nameCallback();
                } else {
    ...
    function stripColorCodes(str: string) {
        if (typeof str === "string")
            return str.replace(/\xA7[0-9A-FK-OR]/ig, "");
        return str;
    }

    function stripColorCodesFromArray(arr: string[]) {
        let stripped = [];
