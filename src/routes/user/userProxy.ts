...
export default function(vars: Vars, pool: Pool) {
    return function (req: Request, res: Response) {
        ...
        function queryUserFromHypixel(existingUser?: any) {
            console.log("Querying Hypixel API for " + user + "...");
            get({
                url: "https://api.hypixel.net/player?key=" + vars.hypixel.key + "&" + (user.length > 16 ? "uuid" : "name") + "=" + user,
                json: true,
                headers: {
                ...
                let formatted: any = {
                    ...
                    rank: player.rank /* youtubers & staff */ || player.monthlyPackageRank/* sunthly subscription (mvp++)*/ || player.newPackageRank || player.packageRank || "NONE",
                    firstLogin: player.firstLogin || 0,
        ...
        function sqlCallback(err: MysqlError | null, results?: any) {
            if (err) {
                console.warn(err);
                res.status(500).json({
                    success: false,
            ...
            if (results.length <= 0) {// Nothing found, try to query the live data from Hypixel API
                queryUserFromHypixel(undefined);
