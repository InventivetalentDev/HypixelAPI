...
module.exports = function (_: Vars, pool: Pool) {
    ...
    setInterval(function () {
        ...
        if (queries && queries.length>0) {
            pool.query(
                "INSERT INTO skyblock_magma_timer_pings (ip,time,active_time,minecraftName,isMod,modName) VALUES ? ON DUPLICATE KEY UPDATE time=values(time), active_time=values(active_time), minecraftName=values(minecraftName), isMod=values(isMod), modName=values(modName)",
