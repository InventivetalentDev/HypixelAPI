...
export default function(vars: Vars, pool: Pool) {
    ...
    return function (req: Request, res: Response) {
        ...
        function continueRequest(captchaRes?: any) {
            /// 2
            ...
            //     }
                ...
                function ipSqlCallback(err: Error | null, results: any) {
                    /// 3
                    if (err) {
                        console.warn(err);
                        res.status(500).json({
                    ...
                    function nameCallback() {
                        ...
                        pool.query(
                            ...
                            [date, type, ipv4, ipv6, username, server, isMod ? 1 : 0, modName, captchaScore], function (err, results) {
                                ...
                                let hash = createHash("md5").update(type + roundedDate.toUTCString()).digest("hex");
                    ...
                    if (username.length > 0) {
                        verifyMinecraftUsername(username, function (err, nameRes) {
                            if (nameRes) {
                                confirmationIncrease += 2;
                                nameCallback();
                            } else {
