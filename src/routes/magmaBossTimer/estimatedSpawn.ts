...
import { postDiscordMessage } from "../../util";
...
export default function(vars: Vars, pool: Pool) {
    ...
    const runner = webhookRunner(pool);

    let latestOneSignalNotification: any;
    ...
    function loadData() {
        ...
        pool.query(
            "SELECT type,time_rounded,confirmations,time_average,time_latest FROM skyblock_magma_timer_events WHERE confirmations >= 30 AND time_rounded >= NOW() - INTERVAL 8 HOUR ORDER BY time_rounded DESC, confirmations DESC LIMIT 20", function (err, results) {
                ...
                let eventTimes: {[key: string]: number} = {
                    "blaze": 0,
                    "magma": 0,
                    "music": 0,
                    "spawn": 0,
                ...
                let eventConfirmations: {[key: string]: number} = {
                    "blaze": 0,
                    "magma": 0,
                    "music": 0,
                    "spawn": 0,
                ...
                if (!results || results.length <= 0) {
                    postDiscordMessage("[MagmaTimer] There is no data!!");
                ...
                if (now - results[0].time_latest.getTime() > threeHoursInMillis * 2) {
                   try{
                       postDiscordMessage("[MagmaTimer] Latest data is older than 6 hours!");
                   }catch (e) {
                       console.warn(e);
                   }
                }
                ...
                if (bestConfirmations < 100) {
                    try{
                        postDiscordMessage("[MagmaTimer] Best confirmation score was " + bestConfirmations);
                    }catch (e) {
                        console.warn(e);
                    }
                }
                ...
                if (!latestOneSignalNotification && prioritizeWaves) {
                    if (minutesUntilNextSpawn <= 10 && minutesUntilNextSpawn >= 8) {
                        ...
                        OneSignalClient.sendNotification(latestOneSignalNotification, function (err: Error, _: any, data: any) {
                            if (err) {
                        ...
                        runner.queryWebhooksAndRun("magmaBoss", data);
                    }
                } else {
